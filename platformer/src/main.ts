import { makeFlameEnemy, makeGuyEnemy, makePlayer, makeBirdEnemy, setControls } from './entities'
...
async function gameSetup() {
  ...
  k.scene('level-1', async () => {
    ...
    for (const bird of level1SpawnPoints.bird) {
      ...
      k.loop(10, () => {
        makeBirdEnemy(k, bird.x, bird.y, speeds[Math.floor(Math.random() * speeds.length)])
      })
    }
  })
